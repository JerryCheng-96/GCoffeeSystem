...
public class GourmetCoffee  {
	...
	public void displayOrder() {
		...
		if (size == 0) {
			...
			for (Iterator i = currentOrder.getItemsIterator(); i.hasNext();) {
				stdOut.println(((OrderItem) i.next()).toString());
			}
	...
	public void displayNumberOfOrders(Product product) {
		Iterator i = this.sales.getOrdersIterator();
		Order order;
		Product nowProduct;
		int cnt = 0;
		...
		while (i.hasNext()) {
			order = (Order)i.next();
			for (Iterator iterator = order.getItemsIterator(); iterator.hasNext();) {
				nowProduct = ((OrderItem)iterator.next()).getProduct();
				if (nowProduct.equals(product)) {
					cnt++;
					break;
				}
			}
		}
		stdOut.println("Numbers of orders that contains the product " + product.getCode() + ": " + cnt);
		
	}

	/**
	 * Displays the total quantity of product that has been sold for each 
	 * product in the catalog.
	 */
	public void displayTotalQuantityOfProducts() {
		Product nowProduct;
		Order nowOrder;
		OrderItem nowOrderItem;
		int quantity = 0;

		for (Iterator productIterator = this.catalog.getProductsIterator(); productIterator.hasNext();) {
			nowProduct = (Product)productIterator.next();
			for (Iterator orderIterator = this.sales.getOrdersIterator(); orderIterator.hasNext();) {
				nowOrder = (Order)orderIterator.next();
				for (Iterator itemsIterator = nowOrder.getItemsIterator(); itemsIterator.hasNext();) {
					nowOrderItem = (OrderItem)itemsIterator.next();
					if (nowOrderItem.getProduct().equals(nowProduct)) {
						quantity += nowOrderItem.getQuantity();
					}
				}
			}
			stdOut.println(nowProduct.getCode() + " " + quantity);
			quantity = 0;
		}
